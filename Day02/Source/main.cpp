...
void Part1()
{
	...
	const uint32_t amountOfRedCubes = 12;
	const uint32_t amountOfGreenCubes = 13;
	const uint32_t amountOfBlueCubes = 14;

	std::unordered_map<uint32_t, Cubes> amountOfCubes;

	// Parse lines --> Splitting strings in C++, amazing..... not...
	std::string line;
	while (std::getline(file, line))
	{
		...
		while (ss >> temp)
		{
			if (temp.find("red") != std::string::npos)
				color = "red";
			if (temp.find("green") != std::string::npos)
				color = "green";
			if (temp.find("blue") != std::string::npos)
				color = "blue";

			if (strtol(temp.c_str(), nullptr, 0) != 0)
				count = atoi(temp.c_str());

			// Check if we have 2 valid values and if so, update the max
			if (count > 0 && !color.empty())
			{
				if (color == "red")
					cubes.Red = std::max(cubes.Red, count);
				if (color == "green")
					cubes.Green = std::max(cubes.Green, count);
				if (color == "blue")
					cubes.Blue = std::max(cubes.Blue, count);
		...
		if (cubes.Red <= amountOfRedCubes && cubes.Green <= amountOfGreenCubes && cubes.Blue <= amountOfBlueCubes)
			amountOfCubes[gameId] = cubes;
	}

	uint32_t sum = 0;
	for (const auto &[gameId, cubes] : amountOfCubes)
		sum += gameId;
...
void Part2()
{
	...
	std::unordered_map<uint32_t, CubesPart2> amountOfCubes;
	...
	while (std::getline(file, line))
	{
		...
		cubes.Power = cubes.Red * cubes.Green * cubes.Blue;
